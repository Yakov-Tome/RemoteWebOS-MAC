...
const powerKey = document.getElementById('powerKey');
...
function showError(message, el) {
  flashErr(el);
  statusEl.textContent = 'שגיאה: ' + message;
  ledEl.className = 'led led-err';
}
...
powerKey.addEventListener('click', async () => {
  const turnOn = !lastStatus.connected;
  const res = await runCmd(turnOn ? 'POST /api/power/on' : 'POST /api/power/off', powerKey);
  if (res !== null) {
    statusEl.textContent = turnOn ? 'שולח Wake-on-LAN...' : 'מכבה...';
...
async function refreshStatus() {
  try {
    ...
    if (s.keyError) {
      ...
      statusEl.textContent = 'שמירת מפתח החיבור נכשלה — האישור יידרש שוב';
      ledEl.className = 'led led-err';
    } else if (s.connected) {
      statusEl.textContent = `מחובר · ${s.ip}`;
      ledEl.className = 'led led-ok';
    } else if (s.awaitingAuth) {
      statusEl.textContent = 'אשר פעם אחת על מסך הטלוויזיה — האישור יישמר';
      ledEl.className = 'led led-warn';
    } else {
      statusEl.textContent = `מחפש את הטלוויזיה · ${s.ip}`;
      ledEl.className = 'led led-warn';
    }
    powerKey.classList.toggle('on-state', !s.connected);
    powerKey.title = s.connected ? 'כיבוי' : 'הדלקה (Wake-on-LAN)';
  } catch {
    lastStatus = { connected: false };
    statusEl.textContent = 'השרת המקומי לא זמין';
    ledEl.className = 'led led-err';
  }
}
